      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              {hasRole("AGRONOMIST") ? "Respond to farmer feedback and questions" : "Manage all feedback responses"}
            </p>
          </div>
          {canCreateResponse && (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Professional responses to farmer feedback and questions</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                ...
                {responses.map((response) => (
                  <TableRow key={response.id}>
                    <TableCell className="max-w-xs">
                      <div className="truncate font-medium">{response.message}</div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <Badge variant="secondary">
                          {response.author?.username || response.author?.email || "Unknown"}
                        </Badge>
                    ...
                    <TableCell className="max-w-xs">
                      <div className="truncate text-muted-foreground">{response.feedback?.comment || "N/A"}</div>
                    </TableCell>
                    <TableCell>
                      {response.feedback?.category && <Badge variant="outline">{response.feedback.category}</Badge>}
                    </TableCell>
                    <TableCell>{new Date(response.createdAt).toLocaleDateString()}</TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        <Button variant="ghost" size="sm" asChild>
                          <Link href={`/dashboard/feedback-responses/${response.id}`}>
